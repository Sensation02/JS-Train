...
function getUsersFromDatabase(array) {
  // Використовуємо метод `Array.from` для створення масиву користувачів зі списку, елементи якого це об'єкти які містять
  ...
  return (array = Array.from(array, (item) => {
    return {
      id: item.id,
...
function customUnshift(arr, ...elements) {
  ...
  // Додаємо елементи на початок масиву за допомогою методу `unshift` та циклу for, початкове значення лічильника на 1 менше ніж довжина масиву,
...
function customAt(arr, index) {
  ...
  // перевіряємо якого типу елемент ми отримали,якщо рядок виведемо у консоль - Елемент є рядком,числом - Елемент є числом, об'єктом - Елемент є об'єктом
...
function customIncludes(arr, element) {
  ...
  if (!Array.isArray(arr) || arr.includes(element) === false) {
    return false
  } else {
