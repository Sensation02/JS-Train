...
function sortUrlParams(url) {
  ...
  // Повертаємо новий URL як рядок.
  const urlObj = new URL(url)
  const params = [...urlObj.searchParams.entries()].sort((a, b) =>
    a[0].localeCompare(b[0]),
  )
  urlObj.search = ''
  params.forEach((param) => urlObj.searchParams.append(param[0], param[1]))
  return urlObj.href
}
...
function getURLValues(url) {
  ...
  const values = []
  for (const param of params) {
    values.push(urlObj.searchParams.getAll(param))
  }
  return values.flat()
}
