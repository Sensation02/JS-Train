...
function Vehicle(brand, model, year, mileage) {
  ...
  const vehicle = Object.assign(this, { brand, model, year, mileage })
  return vehicle
}
...
// Рядковому представленню прототипу Car призначаємо функцію яка повертає рядок: <brand> <model> <year> - <fuelType>.
...
// Створюємо метод accelerate для прискорення швидкості прототипу Car, збільшує this.speed на передане число та виводить рядок в консоль: Автомобіль <make> <model> прискорився до швидкості <speed> км/год
...
// Додатковий метод specific для прототипу Trucks, приймає число якщо воно більше towingCapacity виводить рядок в консоль: Навантаження занадто важке для буксирування, якщо ні то рядок Тягнення навантаження...
...
const audiDrive = audi.drive.bind(audi)
