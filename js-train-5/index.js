...
function printStrings(array) {
  // Перетворюємо масив в рядок і видаляємо пробіли та виводимо його на консоль
...
function addTenToEach(array) {
  ...
  console.log(result)
  // або так: console.log(array.map(value => value + 10)) :)
}
...
function swapMinMax(array) {
  ...
  return array
  // або так: return array.map(value => value === max ? min : value === min ? max : value) :)
}
...
function getEvenNumbers(arr) {
  ...
  return result
  // або так: return arr.filter(num => num % 2 === 0) :)
}
...
function multiplyByIndex(arr) {
  ...
  let result = []
  for (num of arr) {
    result.push(num * arr.indexOf(num))
  }
  return result
  // або так: return arr.map((num, index) => num * index) :)
}
...
function replaceNumbers(arr) {
  ...
  return result
  // або так: return arr.map(num => (num > 10 ? 'Greater than 10' : 'Less than or equal to 10')) :)
}
